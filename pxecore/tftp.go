...
func (s *Server) tftpReadHandler(filename string, rf io.ReaderFrom) error {
	rootPath := filepath.Join(s.Config.Global.DocRoot, s.Config.PXE.TFTPRoot, filename)
	...
	log.Infof("TFTP: tftp_files %d bytes sent", n)
	return nil
}
...
func (s *Server) serveTFTP(l *net.UDPConn) error {
	rootPath := filepath.Join(s.Config.Global.DocRoot, s.Config.PXE.TFTPRoot)
	tftpServer := tftp.NewServer(s.tftpReadHandler, s.tftWriteHandler)
	tftpServer.SetTimeout(5 * time.Second) // optional
	log.Infof("starting tftp server and listening on port %s handle on path: %s", s.Config.PXE.TFTPPort, rootPath)
